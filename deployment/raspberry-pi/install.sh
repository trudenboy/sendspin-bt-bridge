#!/usr/bin/env bash
# install.sh — One-liner installer for Sendspin Bluetooth Bridge
#
# Usage:
...
# Host-side bind-mount target referenced by the canonical docker-compose.yml as
# ``/etc/docker/Sendspin:/config``. The container reads its config.json from
# here; writing it anywhere else is silently ignored.
CONFIG_DIR_HOST="/etc/docker/Sendspin"
...
# Tracks whether subsequent docker commands need a sudo prefix: get.docker.com
# adds the user to the ``docker`` group, but the new group is not active in
# the current shell until logout/login. Without this, ``docker compose pull``
# fails with ``permission denied on /var/run/docker.sock`` and the user is
# left with the misleading impression that the Docker install itself failed.
DOCKER_CMD_PREFIX=""
if ! $DOCKER_INSTALLED; then
  ...
  if docker info &>/dev/null; then
    ok "Docker daemon reachable without re-login"
  elif sudo -n docker info &>/dev/null; then
    DOCKER_CMD_PREFIX="sudo "
    info "New 'docker' group membership not active in this shell — using sudo for the remaining docker steps."
    info "After this run completes, log out and back in (or run 'newgrp docker') so future commands work without sudo."
  else
    abort "Docker is installed but unreachable in this shell. Log out, log back in, and re-run this installer."
  fi
  echo ""

  # Re-check compose (with the prefix decided above)
  if ${DOCKER_CMD_PREFIX}docker compose version &>/dev/null; then
    COMPOSE_OK=true
  fi
fi
...
if ! $COMPOSE_OK; then
  ...
  if command -v docker &>/dev/null && ${DOCKER_CMD_PREFIX}docker compose version &>/dev/null; then
    COMPOSE_OK=true
  fi
fi
...
# Determine timezone — timedatectl is more reliable than /etc/timezone on
# minimal Debian/Ubuntu images, which may ship without the latter.
...
# Sendspin BT Bridge — generated by install.sh on $(date -Iseconds)
AUDIO_UID=${DETECTED_UID}
AUDIO_GID=${DETECTED_GID}
TZ=${TZ_CURRENT}
...
EOF
  ...
  info "  AUDIO_GID=${DETECTED_GID}"
  info "  TZ=${TZ_CURRENT}"
else
  ok "Kept existing .env file"
fi

# Provision the host-side config bind-mount target.  docker-compose.yml mounts
# /etc/docker/Sendspin into the container as /config, and the bridge process
# runs as AUDIO_UID:AUDIO_GID — so the directory must exist and be owned by
# that pair before the container starts.  Without this, the first MA OAuth
# save / password change / log-level toggle fails with a generic 500 and the
# startup banner prints "Config write: ✗ NOT writable".
step "11a. Host config directory ($CONFIG_DIR_HOST)"
if [ -d "$CONFIG_DIR_HOST" ]; then
  ok "Config directory exists: $CONFIG_DIR_HOST"
else
  info "Creating $CONFIG_DIR_HOST (requires sudo)..."
  if sudo mkdir -p "$CONFIG_DIR_HOST"; then
    ok "Created $CONFIG_DIR_HOST"
  else
    abort "Could not create $CONFIG_DIR_HOST — re-run with appropriate privileges."
  fi
fi

CURRENT_OWNER=$(stat -c '%u:%g' "$CONFIG_DIR_HOST" 2>/dev/null || echo "")
if [ "$CURRENT_OWNER" != "${DETECTED_UID}:${DETECTED_GID}" ]; then
  info "Setting owner of $CONFIG_DIR_HOST to ${DETECTED_UID}:${DETECTED_GID} (requires sudo)..."
  if sudo chown -R "${DETECTED_UID}:${DETECTED_GID}" "$CONFIG_DIR_HOST"; then
    ok "Owner of $CONFIG_DIR_HOST set to ${DETECTED_UID}:${DETECTED_GID}"
  else
    skip "Could not chown $CONFIG_DIR_HOST — the bridge may report 'Config write: ✗ NOT writable'"
  fi
else
  ok "Owner of $CONFIG_DIR_HOST is already ${DETECTED_UID}:${DETECTED_GID}"
fi

# Write initial config.json with device (if MAC provided) — into the
# host-side bind-mount target, NOT the project directory.
CONFIG_FILE="${CONFIG_DIR_HOST}/config.json"
...
if $INTERACTIVE && $BT_OK && command -v bluetoothctl &>/dev/null; then
  if ask_yn "Would you like to pair a Bluetooth speaker now? [y/N]" "N"; then
    ...
    # ``bluetoothctl devices`` returns the full known-device cache, not only
    # those that surfaced during the scan we just ran, so the list mixes new
    # discoveries with anything BlueZ already knew about. We call this out in
    # the header so the user isn't surprised to see previously seen devices.
    ...
    if [ -z "$DISCOVERED" ]; then
      ...
      echo -e "  ${BOLD}Available devices (newly discovered or previously seen):${NC}"
      DISC_MACS=()
      DISC_NAMES=()
      IDX=1
      while IFS= read -r line; do
