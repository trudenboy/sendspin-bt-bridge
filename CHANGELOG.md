...
- **Explicit A2DP Sink profile request right after pair succeeds** —
  ...
  fresh from pair narrows that window — on a healthy stack the helper
  returns `AlreadyConnected` and is a cheap no-op. Best-effort: if the
  D-Bus call fails, the pair result is still reported as success and
  `_connect_device_inner` will retry the same hint after its own
  `Connect()`.
