...
def test_load_defaults_when_no_file():
    from sendspin_bridge.config import CONFIG_SCHEMA_VERSION, DEFAULT_CONFIG, load_config
...
def test_load_migrates_legacy_config_keys(tmp_path):
    ...
    from sendspin_bridge.config import CONFIG_SCHEMA_VERSION, load_config
...
def test_save_device_sink_normalizes_mac_and_sink(tmp_path):
    _write_config(tmp_path, {"BLUETOOTH_DEVICES": [{"mac": "AA:BB:CC:DD:EE:FF"}]})
    from sendspin_bridge.config import save_device_sink
...
def test_password_roundtrip():
    from sendspin_bridge.config import check_password, hash_password
...
def test_resolve_device_room_context_prefers_manual_room_metadata_over_ha_area():
    from sendspin_bridge.config import resolve_device_room_context
...
def test_runtime_version_prefers_persisted_install_ref(tmp_path, monkeypatch):
    from sendspin_bridge.config import get_installed_version_ref, get_runtime_version
...
def test_runtime_version_ignores_non_semver_install_ref(tmp_path, monkeypatch):
    from sendspin_bridge.config import VERSION, get_installed_version_ref, get_runtime_version
...
def test_load_config_falls_back_to_stable_for_invalid_update_channel(tmp_path):
    ...
    from sendspin_bridge.config import DEFAULT_UPDATE_CHANNEL, load_config
...
def test_detect_ha_addon_channel_uses_hostname_suffix():
    from sendspin_bridge.config import detect_ha_addon_channel
...
def test_resolve_runtime_ports_follow_installed_addon_track_not_update_channel():
    from sendspin_bridge.config import resolve_base_listen_port, resolve_web_port
...
def test_resolve_additional_web_port_is_disabled_in_ha_addon():
    from sendspin_bridge.config import resolve_additional_web_port
...
def test_load_config_logs_info_only_once_then_debug(tmp_path, caplog):
    ...
    with caplog.at_level("DEBUG", logger="sendspin_bridge.config"):
        config.load_config()
        config.load_config()
...
def test_update_config_logs_debug_for_runtime_state_only_change(tmp_path, caplog):
    from sendspin_bridge.config import CONFIG_SCHEMA_VERSION, update_config
