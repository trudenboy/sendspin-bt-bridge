...
[sendspin-bt-bridge]
    ...
    ├── SendspinClient × N → subprocess(GStreamer pulsesink device=bluez_sink.MAC)
    ├── Flask API
...
Subprocess (`daemon_process.py`) остаётся backend-агностичным: получает имя sink/device и
передаёт его явно в output element GStreamer. Backend обеспечивает готовность назначения до
старта subprocess.
...
class LocalSinkBackend(AudioBackend):
    ...
    def get_sink_name(self) -> str:
        return self.sink_name  # subprocess задаёт pulsesink device=this
```
