...
EOF
  ...
  info "  BLUETOOTH_MAC=${BT_MAC:-<not set>}"
    ...
    if [ -z "$DISCOVERED" ]; then
      ...
      if [ -n "$CHOICE" ] && [ "$CHOICE" -ge 1 ] 2>/dev/null && [ "$CHOICE" -le "${#DISC_MACS[@]}" ] 2>/dev/null; then
        ...
        if bluetoothctl info "$SEL_MAC" 2>/dev/null | grep -q "Connected: yes"; then
          ...
          # Update .env with paired MAC
          if [ -f "$PROJECT_DIR/.env" ]; then
            sed "s/^BLUETOOTH_MAC=.*/BLUETOOTH_MAC=${SEL_MAC}/" "$PROJECT_DIR/.env" > "$PROJECT_DIR/.env.tmp"
            mv "$PROJECT_DIR/.env.tmp" "$PROJECT_DIR/.env"
            ok "Updated BLUETOOTH_MAC in .env"
          fi
        else
