...
- **Веб-интерфейс**: Панель мониторинга в реальном времени и UI конфигурации на порту 8080
...
Аддон предоставляет веб-интерфейс через **HA Ingress** (переброс портов не нужен) и отображается в боковой панели HA.
...
- **Карточки устройств**: Статус соединения, состояние воспроизведения, регулятор громкости, отключение звука
...
- **Страница настройки**: Редактирование адреса сервера, добавление/удаление Bluetooth-устройств
- **Системная информация**: IP-адрес, имя хоста, время работы, статус аудио-синка
