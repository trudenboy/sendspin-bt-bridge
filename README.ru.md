...
- **Современный аудиотракт** — aiosendspin 9 обеспечивает Noise-шифрование и постоянную identity каждой колонки; bridge декодирует FLAC в PCM и передаёт его в GStreamer с явной маршрутизацией `pulsesink device=<sink колонки>`.
- **Опциональное Sendspin-сопряжение** — PIN-сопряжение по умолчанию выключено для автоматического обнаружения и воспроизведения. Его можно сделать обязательным в Configuration; Noise-шифрование транспорта работает и без него.
...
- **Стабильный demo-стенд** — публичное live demo и `DEMO_MODE=true python -m sendspin_bridge` дают повторяемую среду для UI-проверок, скриншотов и демонстраций без Bluetooth-железа.
