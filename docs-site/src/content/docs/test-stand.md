...
This page documents the physical reference deployment used for regression checks and real-hardware validation.

For documentation screenshots and deterministic UI review, prefer the built-in **demo mode** instead of the physical stand:

```bash
DEMO_MODE=true python sendspin_client.py
```

Open `http://127.0.0.1:8080/` after startup. The demo bootstraps a stable six-player fixture with representative diagnostics, logs, onboarding, recovery guidance, and Music Assistant state, which makes it the canonical capture environment for docs-site screenshots.

The matrix below reflects the March 22 docs/runtime snapshot: the stable HA add-on track on HAOS, plus standalone Proxmox and Turris LXC installs. Since v2.55.0, all HA add-on tracks declare `ingress_port: 0` and Supervisor assigns the ingress port at runtime; only the **player listener** ranges still differ between channels (RC `9028+`, beta `9128+`) so the variants can run side-by-side on the same HAOS host. The HAOS node documented here remains the stable track unless noted otherwise.
    ...
    subgraph net["LAN 192.168.10.0/24"]
        ...
        subgraph proxmox["HP ProLiant MicroServer Gen8 — Celeron G1610T 2.3 GHz / 16 GB RAM"]
            ...
            subgraph P_VM["VM 104 haos — QEMU/KVM — 2 vCPU / 6 GB RAM / 64 GB disk"]
                ...
                P_ADDON["SBB v2.64.3 addon<br/>85b1ecde-sendspin-bt-bridge<br/>3 devices / sync group"]
                P_HAOS --> P_ADDON
            end
...
- HA addon tracks now use a Supervisor-assigned ingress port (`ingress_port: 0` in the manifest) — there is no fixed "channel" ingress port any more. The stable track defaults device listeners from `8928` upward
- prerelease HA addon variants, when installed, keep distinct **device-listener** defaults so they don't clash with the stable track on the same host (`9028+` for RC, `9128+` for beta)
