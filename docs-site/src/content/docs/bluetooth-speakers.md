...
Every row links back to the original issue (or to the production fleet inventory in the project's [`CLAUDE.md`](https://github.com/trudenboy/sendspin-bt-bridge/blob/main/CLAUDE.md#configured-bluetooth-devices-production)) so you can read the full diagnostic thread.
...
|---|---|---|
| **IKEA ENEBY20** | Project production fleet ([`CLAUDE.md`](https://github.com/trudenboy/sendspin-bt-bridge/blob/main/CLAUDE.md#configured-bluetooth-devices-production)) | A2DP, multiroom; on `hci0` (CSR8510 A10) |
| **IKEA ENEBY Portable** | Project production fleet | Same family as ENEBY20 |
...
| **Yandex Mini 2** | Project production fleet | On `hci1`; standby quirks documented in [Troubleshooting](/troubleshooting/) |
| **Lenco LS-500** | Project production fleet | A2DP, on `hci1` |
...
| **AfterShokz OpenMove** | Project production fleet (listed but disabled) | Bone-conduction headphones; pair/connect OK, but disabled in the reference config because they're a portable device that wanders out of range. Works while in BT range |
...
The project's own HAOS test stand currently drives 3 enabled speakers in production:

| Speaker | MAC (masked) | Adapter | Static delay |
|---|---|---|---|
| ENEBY20 | `FC:58:FA:EB:08:6C` | `hci0` (CSR8510 A10) | 0 ms |
| Yandex Mini 2 (007 a) | `2C:D2:6B:B8:EC:5B` | `hci1` (CSR8510 A10) | 0 ms |
| Lenco LS-500 | `30:21:0E:0A:AE:5A` | `hci1` | 0 ms |

Three additional speakers (WH-1000XM4, ENEBY Portable, AfterShokz OpenMove) are configured but disabled in this fleet — they're listed in [`CLAUDE.md`](https://github.com/trudenboy/sendspin-bt-bridge/blob/main/CLAUDE.md#configured-bluetooth-devices-production) for traceability.
