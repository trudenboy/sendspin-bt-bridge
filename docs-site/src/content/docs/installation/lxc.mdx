...
<Aside type="caution">
  **CPU baseline:** the bridge's audio path requires an `x86-64-v2` CPU (SSE4.2 + POPCNT) on AMD64 hosts. Older CPUs (AMD Bobcat / Jaguar, pre-Sandy-Bridge Intel) hit a `signal=4` (SIGILL) crash on every daemon spawn. See [Docker › CPU baseline](/installation/docker/#cpu-baseline-x86_64) for the one-line `grep /proc/cpuinfo` check and the list of confirmed-working baselines.
</Aside>
...
The detailed Proxmox walkthrough remains in [`deployment/lxc/README.md`](https://github.com/trudenboy/sendspin-bt-bridge/blob/main/deployment/lxc/README.md).
...
The full OpenWrt-specific guide remains in [`deployment/lxc/openwrt/README.md`](https://github.com/trudenboy/sendspin-bt-bridge/blob/main/deployment/lxc/openwrt/README.md).
