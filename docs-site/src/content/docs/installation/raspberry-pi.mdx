...
## Speaker connects but no sink appears (WirePlumber 0.5 / Trixie)

On Raspberry Pi OS **Trixie** (and any distro shipping **WirePlumber 0.5+**, the PipeWire session manager), a speaker can pair and connect at the BlueZ level while PipeWire never creates the `bluez_output.*` sink — the web UI reports the device as *"missing a sink"* no matter how often you reconnect.

The cause is WirePlumber 0.5's `monitor.bluez.seat-monitoring` setting (enabled by default): it gates Bluetooth node creation on an active logind *seat*, which a headless system never has. The older 0.4-era fix (`with-logind` in `bluetooth.lua.d`) no longer applies — 0.5 uses SPA-JSON `.conf` drop-ins instead.

Disable seat monitoring for the user running PipeWire:
...
Then reconnect the speaker from the web UI. The install script applies this drop-in automatically on new installs when it detects a running WirePlumber (context: issue [#347](https://github.com/trudenboy/sendspin-bt-bridge/issues/347)).
