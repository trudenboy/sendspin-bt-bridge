...
Если bridge уже видит Bluetooth-адаптер, но ни одной колонки ещё не настроено, dashboard показывает **Scan for devices**. Этот shortcut открывает Scan modal, выбирает первый конкретный адаптер и подсвечивает **Start Scan**, но не запускает радиообмен автоматически.
