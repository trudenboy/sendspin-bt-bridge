...
<Aside type="caution">
  **Базовый уровень CPU:** аудио-стек требует CPU уровня `x86-64-v2` (SSE4.2 + POPCNT) на AMD64-хостах. На более старых CPU (AMD Bobcat / Jaguar, Intel до Sandy Bridge) демон будет падать с `signal=4` (SIGILL) при каждом запуске. Проверка одной командой через `grep /proc/cpuinfo` и список подтверждённых baseline'ов — в разделе [Docker › Базовый уровень CPU](/ru/installation/docker/#базовый-уровень-cpu-x86_64).
</Aside>
...
Подробный Proxmox-гайд по-прежнему находится в [`deployment/lxc/README.md`](https://github.com/trudenboy/sendspin-bt-bridge/blob/main/deployment/lxc/README.md).
...
Полный OpenWrt-специфичный гайд находится в [`deployment/lxc/openwrt/README.md`](https://github.com/trudenboy/sendspin-bt-bridge/blob/main/deployment/lxc/openwrt/README.md).
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/trudenboy/sendspin-bt-bridge/main/deployment/lxc/upgrade.sh)
```
