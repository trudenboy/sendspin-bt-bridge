...
Подтверждено [community-тестированием](https://github.com/trudenboy/sendspin-bt-bridge/issues/269#issuecomment-4432922622): WH-1000XM4 на RPi 4 + Trixie (BlueZ 5.82, PipeWire 1.4.2) + bridge v2.70.1 reconnect'ятся стабильно с первой попытки; те же наушники на Bookworm (BlueZ 5.66) с v2.70.1 продолжают ловить collision-паттерн.
...
## Сбой по базовому уровню CPU (SIGILL / signal=4)

**Симптом.** Карточка устройства показывает, что демон постоянно выходит с `code=-4, signal=4`, время жизни 8–14 с. Блок `--- SENDSPIN CONNECTION ---` в диагностическом отчёте подтверждает ту же картину на нескольких циклах перезапуска, при этом Python traceback нигде в логах не появляется. На версиях моста до v2.71.0 эта же проблема выглядела как тихий ~10-секундный restart-loop без сигнала — именно новая daemon-exit observability v2.71.0 впервые делает этот класс багов видимым.

**Причина.** Аудио-стек демона (PyAV / ffmpeg / NumPy) собран в виде wheel-ов под микроархитектуру `x86-64-v2`, что требует минимум **SSE4.2** и **POPCNT**. CPU, которые ниже этого baseline'а (нетбучные AMD Bobcat / Jaguar, Intel до Sandy Bridge, дефолтная CPU-модель `qemu64`), не могут выполнить одну из SIMD-инструкций в этих wheel-ах. Ядро посылает `SIGILL` дочернему процессу демона, и тот завершается до того, как любой Python `try` / `except` успеет отработать.

**Диагностика.** На хосте (не внутри контейнера):

```bash
grep -m1 -E 'sse4_2|popcnt' /proc/cpuinfo
```

Пустой результат — ваш CPU ниже baseline'а. Дополнительно — точная модель CPU и список заявленных feature-флагов:
...
| Ситуация | Что делать |
|---|---|
| Bare-metal x86 ниже baseline'а (AMD Bobcat / Jaguar, Intel до Sandy Bridge) | Переехать на более новое железо — Sandy Bridge (2011) / AMD Bulldozer / Atom Silvermont / Raspberry Pi 4 и новее работают «из коробки». |
| QEMU / KVM-гость | Поставить CPU-модель гостя `host` (пробрасывает фичи хоста) или `qemu64,+sse4.2,+popcnt`. Дефолтный `qemu64` ниже baseline'а. |
| Docker `--platform=linux/amd64` на не-x86 хосте через qemu-user | Убрать флаг `--platform` и дать Docker'у выбрать нативную архитектуру (`linux/arm64` / `linux/arm/v7`). qemu-user не реализует все SIMD-инструкции, на которые рассчитывают wheel-ы. |

Кастомная from-source-пересборка PyAV + NumPy под более широкий baseline технически возможна, но **не публикуется** в виде образа и не поддерживается как штатный путь. Проект рекомендует фиксить железо или CPU-модель VM.
