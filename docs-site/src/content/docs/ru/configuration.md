...
В карточке устройства отдельно отображаются три разных вида данных:

- **BT delay** — рекомендательная задержка AVDTP, которую прошивка колонки сообщает через BlueZ. Это не акустическое измерение.
- **Delay** — применённая компенсация `static_delay_ms`. Кнопки ±10/±50 меняют её на лету через IPC, без перезапуска плеера.
...
Если BlueZ не отдаёт delay report, bridge предлагает стартовое значение с низкой уверенностью по кодеку: 125 мс для SBC/AAC/aptX/LDAC, 40 мс для aptX LL/FastStream и 20 мс для LC3. Рекомендация никогда не применяется автоматически. При подтверждении сохраняются её источник и кодек, поэтому новое наблюдение не перезапишет откалиброванное значение молча.
...
**Test clicks** проигрывает ограниченный click track прямо в выбранный Bluetooth sink. **Mic compare** — экспериментальная относительная калибровка двух колонок: включите *Microphone latency calibration beta*, откройте UI через HTTPS или localhost, не перемещайте телефон и дождитесь трёх пар измерений. Записи хранятся только в памяти не более десяти минут. Результат добавляет компенсацию более быстрой колонке и не является сертифицированным абсолютным end-to-end измерением.
