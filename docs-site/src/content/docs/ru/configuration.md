...
| **ma_api_url** | Базовый URL REST API Music Assistant (например `http://192.168.1.10:8123`). Необходим для метаданных now-playing, транспортных кнопок и группового воспроизведения через MA. |
| **ma_api_token** | Долгосрочный токен доступа Home Assistant для MA API. Создайте в HA: **Профиль → Долгосрочные токены доступа**. |
...
| `MA_API_URL` | string | `""` | Базовый URL REST API Music Assistant (например `http://192.168.1.10:8123`). Необходим для метаданных now-playing и транспортных кнопок |
| `MA_API_TOKEN` | string | `""` | Долгосрочный токен доступа HA для MA API. Создайте в HA: **Профиль → Долгосрочные токены доступа** |
