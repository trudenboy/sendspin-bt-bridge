...
Если оставить поля портов пустыми, standalone-режим использует **8080** для web UI и **8928** для listener'ов плееров. В режиме Home Assistant addon все три канала (stable / rc / beta) поставляются с `ingress_port: 0`, поэтому Supervisor назначает свободный порт во время запуска — это изменение [появилось в v2.55.0](https://github.com/trudenboy/sendspin-bt-bridge/pull/138), чтобы варианты аддона перестали конфликтовать с Matter / Thread и другими addons на host-network. Per-channel дефолты для базового listener-порта (`8928 / 9028 / 9128`) сохраняются — они нужны, чтобы listener'ы устройств не пересекались, когда несколько вариантов аддона работают одновременно. `WEB_PORT`, заданный внутри аддона, игнорируется; UI открывается из боковой панели Home Assistant.
...
### Задержка и синхронизация отдельных колонок

В интерфейсе отдельно отображаются три разных вида данных:

- **BT delay** — рекомендательная задержка AVDTP, которую прошивка колонки сообщает через BlueZ. Это не акустическое измерение.
- **Delay** — применённая компенсация `static_delay_ms`. Компактный степпер находится в **Configuration → Devices** непосредственно перед Bluetooth actions. Выберите общий шаг ±10/±50 и меняйте значение кнопками минус/плюс; оно применяется через IPC без перезапуска плеера.
- **Метрики backend/buffer/clock** поступают из работающего аудиотракта Sendspin и публикуются в диагностике и Home Assistant. Они описывают хост и синхронизацию часов, но не скрытую задержку DSP внутри колонки.

Если BlueZ не отдаёт delay report, bridge один раз при первой регистрации устройства применяет стартовое значение по кодеку: 125 мс для SBC/AAC/aptX/LDAC, 40 мс для aptX LL/FastStream и 20 мс для LC3. После инициализации ручная настройка и микрофонная калибровка становятся приоритетными, а последующие наблюдения не перезаписывают пользовательское значение.

В advanced-части строки есть **Startup lead** (`required_lead_time_ms`) и **Minimum buffer** (`min_buffer_ms`). Совместимые версии `aiosendspin` применяют оба параметра на лету. Увеличивайте их при частых re-anchor, когда Bluetooth/QEMU-тракт исчерпывает стандартный запас 250 мс; эти настройки не заменяют `static_delay_ms`.

Кнопка метронома запускает непрерывный синхронизированный click track прямо через выбранный Bluetooth sink. Кнопка микрофона запускает относительную калибровку двух колонок: откройте UI через HTTPS или localhost, не перемещайте телефон и дождитесь трёх пар измерений. Записи хранятся только в памяти не более десяти минут. Результат добавляет компенсацию более быстрой колонке и не является сертифицированным абсолютным end-to-end измерением.
