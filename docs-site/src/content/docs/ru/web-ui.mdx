...
|---|---|
| **General** | Имя bridge, timezone, прямой web-порт, base listener port, smooth restart, update policy, видимость guidance и переключатель advanced compatibility tools |
...
Помимо основных полей на вкладке General есть переключатель **Advanced compatibility tools**. Эта UI-only настройка хранится в браузере и открывает узконаправленные recovery- и pairing-обходы. Стабильные функции — имена адаптеров из HA areas, room labels, RSSI, временное отключение соседних колонок и микрофонная калибровка — видны без него.

Для compatibility-настроек UI показывает необходимость рестарта и поддержку текущей платформой. Недоступный PulseAudio- или adapter-recovery-механизм блокируется с конкретной причиной.
...
- **Device fleet** — основная таблица с enabled, именем, MAC, адаптером, портом, компактной live-панелью delay и live badge.
- В деталях строки доступны preferred format, `listen_host`, `keepalive_interval`, а также необязательные **room name / room ID**. Они могут быть получены из HA area или заданы как локальные метки и сами по себе не включают room-handoff Music Assistant.
...
- Карточка **Advanced recovery workarounds** (видна только при включённом **Advanced compatibility tools**) — таблица ниже.
...
- выбор конкретного адаптера для сканирования (первый обнаруженный уже выбран);
...
#### Расширенные Bluetooth compatibility tools

Видны только при включённом **Advanced compatibility tools**:
...
| `EXPERIMENTAL_PA_MODULE_RELOAD` | Перезагружает PulseAudio-модуль `module-bluez5-discover`, если BT-карта не появилась. На PipeWire недоступен и затрагивает все Bluetooth-устройства classic PulseAudio. |
| `EXPERIMENTAL_ADAPTER_AUTO_RECOVERY` | На пороге reconnect failures проверяет rfkill, выполняет MGMT/HCI power-cycle и при неудаче может сбросить USB-адаптер. Нужны Linux capabilities `NET_ADMIN` + `NET_RAW`; для USB reset нужен writable-доступ к USB devices. UI показывает доступный уровень. |
...
В модалке **Scan nearby** также есть настройки конкретной попытки сопряжения:

| Переключатель | Что делает |
|---|---|
| **Temporarily disconnect other speakers** (peer-quiesce) | Стабильная pairing-помощь: отключает соседние колонки на выбранном адаптере; после попытки их восстанавливает обычный reconnect-механизм. |
| **NoInputNoOutput pair agent** | Одноразовый Just-Works для колонок без SSP confirmation. Убирает authenticated numeric comparison, скрыт за compatibility toggle, сбрасывается после запроса, а BlueZ-агент отклоняет обращения от любого MAC кроме выбранного. |
| **Authorize HFP/HSP for this pair** | Одноразовая авторизация сервисов для multi-profile устройств, отказывающихся завершать pairing. Воспроизведение остаётся A2DP-only; HFP backend эта опция не устанавливает. |
...
- **No Bluetooth devices configured** → **Scan for devices** открывает Scan modal, выбирает конкретный адаптер и подсвечивает **Start Scan**, но не запускает радиообмен без подтверждения пользователя.
