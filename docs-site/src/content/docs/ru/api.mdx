...
| Поле | Описание |
|---|---|
| `title` | Заголовок issue (5–200 символов) |
| `description` | Описание (10–5000 символов) |
| `email` | Контактный email (должен содержать `@`) |
| `diagnostics_text` | Опционально: замаскированный текст диагностики (обрезается до лимита GitHub в 65 536 символов) |
