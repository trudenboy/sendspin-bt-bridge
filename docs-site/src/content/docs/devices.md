...
- **`static_delay_ms`** — declares how much extra latency this speaker's hardware adds *beyond* the PulseAudio / PipeWire sink the bridge writes into (Bluetooth radio, codec, speaker DSP, amplifier). Per the [Sendspin protocol](https://www.sendspin-audio.com/spec/#client--server-clientstate-player-object), the bridge subtracts this value from each chunk's play-time so audio is handed off to the kernel **earlier** by exactly `static_delay_ms`; the hardware then adds its real latency back and the acoustic output lands on the scheduled timestamp. The web UI pre-fills **300 ms** for every newly added device because that is a reasonable starting estimate of post-sink BT pipeline latency on typical consumer speakers, and field reports showed it gives a noticeably better baseline than `0` for A/V sync — especially on Ubuntu / PipeWire hosts. Raise the value for speakers that consistently play **behind** the rest of a group (audible as their output trailing the others by a few hundred ms or more) — that means their real hardware latency exceeds what `static_delay_ms` currently compensates. Lower it for speakers that play **ahead**. Negative values are not accepted — sendspin 7.0+ rejects them, and any legacy negative value in an imported config is clamped to `0` at migration time. The range is `0`–`5 000 ms`; cheap BT speakers with large internal jitter buffers may need values approaching 1 000 ms.
...
**Why the raw number does not transfer 1 : 1.** MassDroid measures the *full* acoustic round-trip (DAC + transport + air + microphone capture). Sendspin's DAC-anchored sync already absorbs the parts of the pipeline that PulseAudio / PipeWire reports, plus the mic-capture leg is yours, not the bridge's. What is transferable is the **ranking** and the **deltas** between speakers: in a group, the speaker with the **highest** measured RTT has the most uncompensated hidden latency and should carry the largest `static_delay_ms`; lower-RTT speakers need a smaller value.
...
4. Note each speaker's reported BT delay. The speaker with the **highest** value is the slowest one — it has the most uncompensated hardware latency past the sink.
5. As a starting point, raise `static_delay_ms` on each speaker by roughly `this_rtt − min_rtt` on top of the bridge default, so every speaker advertises a delay close to its real-world hardware latency. For example, if one speaker measures 180 ms and another 260 ms, try setting the 260 ms one ~80 ms higher than the 180 ms one.
