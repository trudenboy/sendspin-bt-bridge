...
| Tab | Purpose |
|---|---|
| **General** | Bridge identity, timezone, direct web port, base listener port, restart behavior, update policy, guidance visibility, and advanced compatibility-tools toggle |
...
Beyond the core identity and policy fields listed in the table, the General tab includes an **Advanced compatibility tools** toggle. This browser-local preference reveals narrowly targeted recovery and pairing workarounds; it is not written to the config file. Stable controls such as adapter-area naming, room labels, RSSI, pairing peer-quiesce, and microphone calibration remain visible without it.

Compatibility rows show their restart requirement and runtime support. An unavailable PulseAudio or adapter-recovery control is disabled with the detected reason instead of accepting a setting that cannot work on the current host.
...
- **Enabled**, **player name**, **MAC**, **adapter**, **port**, a compact live delay panel, **live badge**, and **remove** all live here.
- Per-speaker details include **preferred audio format**, **`listen_host`**, **`keepalive_interval`**, and optional **room name / room ID** context. Room values may come from a Home Assistant area or be entered as local labels; they do not imply Music Assistant room handoff.
...
- **Advanced recovery workarounds** card (visible only with **Advanced compatibility tools** turned on) — see the table further down.

The scan modal improves first-run discovery and troubleshooting:

- choose the specific adapter that should perform this scan (the first detected adapter is preselected);
...
#### Advanced Bluetooth compatibility tools

Visible only with **Advanced compatibility tools** turned on:
...
| `EXPERIMENTAL_PA_MODULE_RELOAD` | Reloads PulseAudio's `module-bluez5-discover` if a Bluetooth card never appears. It is unavailable on PipeWire and disrupts all classic-PulseAudio Bluetooth devices. |
| `EXPERIMENTAL_ADAPTER_AUTO_RECOVERY` | At the reconnect-failure threshold, validates rfkill state, power-cycles the controller through MGMT/HCI, and optionally resets a USB adapter if the power-cycle fails. Requires Linux `NET_ADMIN` + `NET_RAW`; USB reset additionally needs writable USB-device access. The UI reports the detected level. |
...
The scan modal also has pairing-attempt controls:

| Toggle | What it does |
|---|---|
| **Temporarily disconnect other speakers** (peer-quiesce) | Stable pairing aid: disconnects peers on the selected adapter, then lets their normal reconnect paths restore them after the attempt. |
| **NoInputNoOutput pair agent** | One-shot Just-Works mode for speakers that never send SSP confirmation. It removes authenticated numeric comparison, is hidden behind the compatibility toggle, resets after the request, and the BlueZ agent rejects requests for every MAC except the selected target. |
| **Authorize HFP/HSP for this pair** | One-shot service authorization for multi-profile devices that otherwise refuse pairing. Playback remains A2DP-only; this does not install an HFP audio backend. It resets after the request. |
